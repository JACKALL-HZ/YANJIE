...
/**
 * GalaxyBackground — 移植素材库 v2 稿的 Galaxy WebGL 星图（React Bits / ogl）。
 * 黑底 + 高亮白星 + 强光晕十字芒 + 鼠标斥力 + 慢速自转。
 * reduced-motion 渲染单帧静止；触屏关闭鼠标交互。
 */
  ...
  try {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uGlowIntensity: { value: 0.78 },
        uSaturation: { value: 0.06 },
        uMouseRepulsion: { value: true },
        uTwinkleIntensity: { value: 0.48 },
        uRotationSpeed: { value: 0.06 },
        uRepulsionStrength: { value: 2.4 },
    ...
    function update(t: number) {
      ...
      ;(program.uniforms.uMouse.value as Float32Array)[0] = smoothMousePos.x
      ;(program.uniforms.uMouse.value as Float32Array)[1] = smoothMousePos.y
    ...
    // WebGL 不可用时静默降级：atmos + grain 仍在，背景依然成立
  }
})
...
<template>
  <div ref="containerRef" class="pointer-events-none fixed inset-0 -z-[9]" aria-hidden="true" />
</template>
